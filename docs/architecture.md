...
### ADR-001: Use pyRevit Instead of C# Plugin

**Decision**: Build with pyRevit (Python) instead of C# Revit SDK plugin

**Rationale**:
- **Faster iteration**: Python allows script changes without recompiling
- **Easier LLM integration**: Python has better Claude SDK support than .NET
- **Lower barrier to entry**: Python is more accessible than C#
- **pyRevit maturity**: Proven framework with active community

**Trade-offs**:
- Performance: Python slightly slower than C#, but acceptable for this use case
- Deployment: Requires pyRevit installation (but it's a one-time setup)
